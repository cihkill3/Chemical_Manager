...
def _pdf_text(content: bytes) -> str:
    if len(content) < 1_000 or not content.startswith(b"%PDF"):
        raise QualityDocumentError("응답이 유효한 PDF가 아닙니다.")
    try:
        import pymupdf

        doc = pymupdf.open(stream=content, filetype="pdf")
        if doc.page_count < 1:
            raise QualityDocumentError("PDF에 페이지가 없습니다.")
        ...
        raise QualityDocumentError(f"PDF 파싱에 실패했습니다: {exc}") from exc
...
def _verify_pdf(
    ...
    *,
    ...
    if _compact(catalog) not in compact:
        raise QualityDocumentError(f"PDF 본문에서 catalog {catalog!r}를 확인하지 못했습니다.")
    if lot and _compact(lot) not in compact:
        raise QualityDocumentError(f"PDF 본문에서 lot {lot!r}를 확인하지 못했습니다.")
    if require_coa and not any(
        marker in compact
        for marker in ("certificateofanalysis", "certificateofanalyses", "analysiscertificate")
    ):
        raise QualityDocumentError("PDF 본문에서 Certificate of Analysis 표기를 확인하지 못했습니다.")
...
def _http_pdf(url: str, *, referer: str = "") -> bytes:
    ...
    if not response.content.startswith(b"%PDF"):
        raise QualityDocumentError(
            f"PDF 대신 {response.headers.get('content-type', 'unknown')} 응답을 받았습니다: {url}"
        )
...
def _capture_window_open(context, selector: str, *, timeout_seconds: int = 15) -> str:
    ...
    if not result.get("ok"):
        raise QualityDocumentError(result.get("error", "문서 URL을 얻지 못했습니다."))
    return result["url"]
    ...
    for brand in candidates:
        ...
        if not any(marker in title for marker in ("404", "not found", "error")):
            ...
            if _compact(clean) in _compact(page_text):
                return url
    raise QualityDocumentError(f"Sigma-Aldrich 제품 페이지를 찾지 못했습니다: {catalog}")
...
def _aldrich_target(context, lot: str) -> str:
    ...
    if not context.execute_script(marker_script):
        raise QualityDocumentError(f"Sigma-Aldrich 페이지에 lot {lot}가 없습니다.")
...
def _aldrich_search_target(context, catalog: str, lot: str) -> tuple[str, str]:
    ...
    for candidate in dict.fromkeys(candidates):
        ...
        try:
            ...
            continue
    raise QualityDocumentError(
        f"Sigma-Aldrich 공식 COA 검색에 결과가 없습니다: {catalog} / {lot}"
    )
...
def _print_current_page_pdf(context, source_url: str, catalog: str, lot: str) -> bytes:
    ...
    if _compact(catalog) not in _compact(body) or _compact(lot) not in _compact(body):
        raise QualityDocumentError("HTML COA 페이지에서 요청 catalog/lot를 확인하지 못했습니다.")
    if "/documents/coa/" not in source_url.casefold():
        raise QualityDocumentError("열린 HTML 문서의 공식 COA 경로를 확인하지 못했습니다.")
...
def _download_thermofisher(context, catalog: str, lot: str, folder: Path) -> QualityDocument:
    ...
    try:
        ...
        raise QualityDocumentError("Thermo Fisher 인증서 API 응답을 해석하지 못했습니다.") from exc
    matches = [
        asset
...
def _download_abcam(context, catalog: str, lot: str, folder: Path) -> QualityDownloadResult:
    ...
    return QualityDownloadResult(
        ...
        documents=documents,
        warnings=[
            "Abcam COA를 찾지 못해 Datasheet와 lot별 CoC를 저장했습니다. "
            "CoC는 시험 결과가 기재된 COA와 동일한 문서가 아닙니다."
        ],
    )
...
def download_coa(context, vendor: str, catalog: str, lot: str, output_dir: str | Path) -> str:
    ...
    raise QualityDocumentError(
        f"{vendor} {catalog}/{lot}에는 검증된 COA가 없습니다. "
        "download_quality_documents()로 대체 문서 상태를 확인하십시오."
    )
